			...
			<view class="list-box6" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				...
				<view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"0 0 20rpx","backgroundColor":"#fff","overflow":"hidden","borderRadius":"30rpx","width":"48%","height":"auto"}'>
				  <view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"6rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
				  	<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheliangpinpai','修改')) || (!userid && isAuthFront('cheliangpinpai','修改'))" @click.stop="onUpdateTap(product.id)">
				  	...
				  	<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheliangpinpai','删除')) || (!userid && isAuthFront('cheliangpinpai','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`cheliangpinpai`, params);
                } else {
                    res = await this.$api.list(`cheliangpinpai`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('cheliangpinpai', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`cheliangpinpai`, searchForm);
                } else {
                    res = await this.$api.list(`cheliangpinpai`, searchForm);
                }
				// 如果是第一页数据置空
