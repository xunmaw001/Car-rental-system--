...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','cheliangyanse');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','cheliangyanse');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('cheliangyanse', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusscheliangyanse/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
