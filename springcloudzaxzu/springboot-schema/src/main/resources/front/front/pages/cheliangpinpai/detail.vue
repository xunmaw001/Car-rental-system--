...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','cheliangpinpai');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','cheliangpinpai');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('cheliangpinpai', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusscheliangpinpai/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
